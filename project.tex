...
\begin{problem}
	TTraining the model \\
	Train a model with your implementation of the LDA inference algorithm. In the beginning, use a properly sized subset of your whole document corpus. Since the key problem with unsupervised learning techniques like LDA is the validation of your results, you will need manual validation to make sense of the results. Additionally there are various result visualization tools like \href{https://github.com/bmabey/pyLDAvis}{pyLDAvis} \cite{url:python-pyldavisl} that deliver great insights into your training results (see \ref{fig:pyldavis}).
\end{problem}
...
\begin{problem}
	...
	Given your model we want to be able to answer queries of the form: Given article $A$, what are the $k$ most similar articles $B_1, ...,B_k$. As a use case, you can think of the article recommendation for users of the Wikipedia. \\
